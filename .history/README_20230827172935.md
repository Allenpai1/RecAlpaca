...
- We take each user's infomration on real datasets MovieLens, Book-Crossing and Anime to generate our input prompts. 
...
- To teach RecAlpaca to learn the user's perference and user's profile and selected most relevent items in the candidate set we trained two lightGCN network on MovieLens-1m and MovieLens-100k to generate a candidat set and top1 recommendation for each user and generate more detailed instruction-datasets.
